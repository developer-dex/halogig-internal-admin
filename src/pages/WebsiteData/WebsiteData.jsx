...
const WebsiteData = () => {
  ...
  const handleEdit = (record) => {
    ...
    // Pre-populate form data with existing record
    setFormData({
      categoryName: record.category_name || '',
      serviceName: record.service_name || '',
      slugLink: record.slug_link || '',
      primaryKeyword: record.primary_keyword || '',
      secondaryKeyword: record.secondary_keyword || '',
      bannerTitle: record.banner_title || '',
      bannerDescription: record.banner_description || '',
      ...
      industryLists: record.industry_lists 
        ...
        : [''],
      mainApplicationTitle: record.main_application_title || '',
      mainApplicationDescription: record.main_application_description || '',
      mainApplicationLists: record.main_application_lists && record.main_application_lists.length > 0 
        ? record.main_application_lists 
        : [{ title: '', description: '' }],
      interlinkPages: record.interlink_pages && record.interlink_pages.length > 0 
        ? record.interlink_pages 
        : [{ slug: '', altername_skill_name: '' }],
      usercaseListes: record.usercase_listes 
        ? record.usercase_listes.split(',').map(item => item.trim())
        : [''],
  ...
  return (
    <div className="website-data-container">
      ...
      <Dialog 
        ...
        fullWidth
      >
        <DialogTitle>Edit Website Data</DialogTitle>
        <DialogContent>
          <Box mt={2}>
            <Grid container spacing={3}>
              {/* Basic Information */}
